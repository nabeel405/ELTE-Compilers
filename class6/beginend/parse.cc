// Generated by Bisonc++ V5.02.00 on Mon, 19 Oct 2020 12:33:54 +0200
...
PI__ const s_productionInfo[] = 
{
     ...
     {287, 6}, // 1: program (T_PROGRAM) ->  T_PROGRAM T_ID declarations T_BEGIN statements T_END
     {288, 0}, // 2: declarations ->  <empty>
     {288, 2}, // 3: declarations ->  declaration declarations
     {290, 3}, // 4: declaration (T_INTEGER) ->  T_INTEGER T_ID T_SEMICOLON
     {290, 3}, // 5: declaration (T_BOOLEAN) ->  T_BOOLEAN T_ID T_SEMICOLON
     {289, 1}, // 6: statements ->  statement
     {289, 2}, // 7: statements ->  statement statements
     {291, 2}, // 8: statement (T_SKIP) ->  T_SKIP T_SEMICOLON
     {291, 1}, // 9: statement ->  assignment
     {291, 1}, // 10: statement ->  read
     {291, 1}, // 11: statement ->  write
     {291, 1}, // 12: statement ->  branch
     {291, 1}, // 13: statement ->  loop
     {292, 4}, // 14: assignment (T_ID) ->  T_ID T_ASSIGN expression T_SEMICOLON
     {293, 5}, // 15: read (T_READ) ->  T_READ T_OPEN T_ID T_CLOSE T_SEMICOLON
     {294, 5}, // 16: write (T_WRITE) ->  T_WRITE T_OPEN expression T_CLOSE T_SEMICOLON
     {295, 5}, // 17: branch (T_IF) ->  T_IF expression T_BEGIN statements T_END
     {295, 9}, // 18: branch (T_IF) ->  T_IF expression T_BEGIN statements T_END T_ELSE T_BEGIN statements T_END
     {296, 5}, // 19: loop (T_WHILE) ->  T_WHILE expression T_BEGIN statements T_END
     {297, 1}, // 20: expression (T_NUM) ->  T_NUM
     {297, 1}, // 21: expression (T_TRUE) ->  T_TRUE
     {297, 1}, // 22: expression (T_FALSE) ->  T_FALSE
     {297, 1}, // 23: expression (T_ID) ->  T_ID
     {297, 3}, // 24: expression (T_ADD) ->  expression T_ADD expression
     {297, 3}, // 25: expression (T_SUB) ->  expression T_SUB expression
     {297, 3}, // 26: expression (T_MUL) ->  expression T_MUL expression
     {297, 3}, // 27: expression (T_DIV) ->  expression T_DIV expression
     {297, 3}, // 28: expression (T_MOD) ->  expression T_MOD expression
     {297, 3}, // 29: expression (T_LESS) ->  expression T_LESS expression
     {297, 3}, // 30: expression (T_GR) ->  expression T_GR expression
     {297, 3}, // 31: expression (T_EQ) ->  expression T_EQ expression
     {297, 3}, // 32: expression (T_AND) ->  expression T_AND expression
     {297, 3}, // 33: expression (T_OR) ->  expression T_OR expression
     {297, 2}, // 34: expression (T_NOT) ->  T_NOT expression
     {297, 3}, // 35: expression (T_OPEN) ->  T_OPEN expression T_CLOSE
     {298, 1}, // 36: program_$ ->  program
};
