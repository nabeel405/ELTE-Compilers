// Generated by Bisonc++ V5.02.00 on Tue, 27 Oct 2020 23:11:46 +0100
...
PI__ const s_productionInfo[] = 
{
     ...
     {293, 0}, // 4: declarations ->  <empty>
     {295, 3}, // 5: declaration (T_INTEGER) ->  T_INTEGER T_ID T_SEMICOLON
     {295, 3}, // 6: declaration (T_BOOLEAN) ->  T_BOOLEAN T_ID T_SEMICOLON
     {295, 3}, // 7: declaration (T_INTEGER) ->  T_INTEGER variables T_SEMICOLON
     {295, 3}, // 8: declaration (T_BOOLEAN) ->  T_BOOLEAN variables T_SEMICOLON
     {296, 1}, // 9: variables (T_ID) ->  T_ID
     {296, 3}, // 10: variables (T_ID) ->  T_ID T_COMMA variables
     {294, 1}, // 11: statements ->  statement
     {294, 2}, // 12: statements ->  statement statements
     {297, 2}, // 13: statement (T_SKIP) ->  T_SKIP T_SEMICOLON
     {297, 1}, // 14: statement ->  assignment
     {297, 1}, // 15: statement ->  read
     {297, 1}, // 16: statement ->  write
     {297, 1}, // 17: statement ->  branch
     {297, 1}, // 18: statement ->  loop
     {298, 4}, // 19: assignment (T_ID) ->  T_ID T_ASSIGN expression T_SEMICOLON
     {299, 5}, // 20: read (T_READ) ->  T_READ T_OPEN T_ID T_CLOSE T_SEMICOLON
     {300, 5}, // 21: write (T_WRITE) ->  T_WRITE T_OPEN expression T_CLOSE T_SEMICOLON
     {301, 5}, // 22: branch (T_IF) ->  T_IF expression T_THEN statements T_ENDIF
     {301, 7}, // 23: branch (T_IF) ->  T_IF expression T_THEN statements T_ELSE statements T_ENDIF
     {302, 5}, // 24: loop (T_WHILE) ->  T_WHILE expression T_DO statements T_DONE
     {303, 1}, // 25: expression (T_NUM) ->  T_NUM
     {303, 1}, // 26: expression (T_TRUE) ->  T_TRUE
     {303, 1}, // 27: expression (T_FALSE) ->  T_FALSE
     {303, 1}, // 28: expression (T_ID) ->  T_ID
     {303, 3}, // 29: expression (T_ADD) ->  expression T_ADD expression
     {303, 3}, // 30: expression (T_SUB) ->  expression T_SUB expression
     {303, 3}, // 31: expression (T_MUL) ->  expression T_MUL expression
     {303, 3}, // 32: expression (T_DIV) ->  expression T_DIV expression
     {303, 3}, // 33: expression (T_MOD) ->  expression T_MOD expression
     {303, 3}, // 34: expression (T_LESS) ->  expression T_LESS expression
     {303, 3}, // 35: expression (T_GR) ->  expression T_GR expression
     {303, 3}, // 36: expression (T_EQ) ->  expression T_EQ expression
     {303, 3}, // 37: expression (T_AND) ->  expression T_AND expression
     {303, 3}, // 38: expression (T_OR) ->  expression T_OR expression
     {303, 2}, // 39: expression (T_NOT) ->  T_NOT expression
     {303, 3}, // 40: expression (T_OPEN) ->  T_OPEN expression T_CLOSE
     {304, 1}, // 41: program_$ ->  program
};
