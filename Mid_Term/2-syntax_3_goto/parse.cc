// Generated by Bisonc++ V5.02.00 on Wed, 28 Oct 2020 00:23:37 +0100
...
PI__ const s_productionInfo[] = 
{
     ...
     {297, 3}, // 6: declaration (T_BOOLEAN) ->  T_BOOLEAN T_LABEL T_SEMICOLON
     {297, 3}, // 7: declaration (T_INTEGER) ->  T_INTEGER T_LABEL T_SEMICOLON
     {296, 1}, // 8: statements ->  statement
     {296, 2}, // 9: statements ->  statement statements
     {298, 2}, // 10: statement (T_SKIP) ->  T_SKIP T_SEMICOLON
     {298, 1}, // 11: statement ->  assignment
     {298, 1}, // 12: statement ->  read
     {298, 1}, // 13: statement ->  write
     {298, 1}, // 14: statement ->  branch
     {298, 1}, // 15: statement ->  loop
     {298, 1}, // 16: statement ->  goto
     {298, 1}, // 17: statement ->  label
     {304, 3}, // 18: goto (T_GOTO) ->  T_GOTO T_LABEL T_SEMICOLON
     {305, 2}, // 19: label (T_LABEL) ->  T_LABEL T_COLON
     {299, 4}, // 20: assignment (T_ID) ->  T_ID T_ASSIGN expression T_SEMICOLON
     {300, 5}, // 21: read (T_READ) ->  T_READ T_OPEN T_ID T_CLOSE T_SEMICOLON
     {301, 5}, // 22: write (T_WRITE) ->  T_WRITE T_OPEN expression T_CLOSE T_SEMICOLON
     {302, 5}, // 23: branch (T_IF) ->  T_IF expression T_THEN statements T_ENDIF
     {302, 7}, // 24: branch (T_IF) ->  T_IF expression T_THEN statements T_ELSE statements T_ENDIF
     {303, 5}, // 25: loop (T_WHILE) ->  T_WHILE expression T_DO statements T_DONE
     {306, 1}, // 26: expression (T_NUM) ->  T_NUM
     {306, 1}, // 27: expression (T_TRUE) ->  T_TRUE
     {306, 1}, // 28: expression (T_FALSE) ->  T_FALSE
     {306, 1}, // 29: expression (T_LABEL) ->  T_LABEL
     {306, 1}, // 30: expression (T_ID) ->  T_ID
     {306, 3}, // 31: expression (T_ADD) ->  expression T_ADD expression
     {306, 3}, // 32: expression (T_SUB) ->  expression T_SUB expression
     {306, 3}, // 33: expression (T_MUL) ->  expression T_MUL expression
     {306, 3}, // 34: expression (T_DIV) ->  expression T_DIV expression
     {306, 3}, // 35: expression (T_MOD) ->  expression T_MOD expression
     {306, 3}, // 36: expression (T_LESS) ->  expression T_LESS expression
     {306, 3}, // 37: expression (T_GR) ->  expression T_GR expression
     {306, 3}, // 38: expression (T_EQ) ->  expression T_EQ expression
     {306, 3}, // 39: expression (T_AND) ->  expression T_AND expression
     {306, 3}, // 40: expression (T_OR) ->  expression T_OR expression
     {306, 2}, // 41: expression (T_NOT) ->  T_NOT expression
     {306, 3}, // 42: expression (T_OPEN) ->  T_OPEN expression T_CLOSE
     {307, 1}, // 43: program_$ ->  program
};
