// Generated by Bisonc++ V5.02.00 on Tue, 03 Nov 2020 23:07:51 +0100
...
PI__ const s_productionInfo[] = 
{
     ...
     {288, 6}, // 1: program (T_PROGRAM) ->  T_PROGRAM T_ID declarations T_BEGIN statements T_END
     {289, 0}, // 2: declarations ->  <empty>
     {289, 2}, // 3: declarations ->  declaration declarations
     {291, 3}, // 4: declaration (T_INTEGER) ->  T_INTEGER T_ID T_SEMICOLON
     {291, 3}, // 5: declaration (T_BOOLEAN) ->  T_BOOLEAN T_ID T_SEMICOLON
     {290, 1}, // 6: statements ->  statement
     {290, 2}, // 7: statements ->  statement statements
     {292, 2}, // 8: statement (T_SKIP) ->  T_SKIP T_SEMICOLON
     {292, 1}, // 9: statement ->  assignment
     {292, 1}, // 10: statement ->  read
     {292, 1}, // 11: statement ->  write
     {292, 1}, // 12: statement ->  branch
     {292, 1}, // 13: statement ->  loop
     {292, 2}, // 14: statement (T_SEMICOLON) ->  simultenous T_SEMICOLON
     {298, 3}, // 15: simultenous (T_ASSIGN) ->  names T_ASSIGN values
     {299, 1}, // 16: names (T_ID) ->  T_ID
     {299, 3}, // 17: names (T_ID) ->  T_ID T_COMMA names
     {300, 1}, // 18: values ->  expression
     {300, 3}, // 19: values (T_COMMA) ->  expression T_COMMA values
     {293, 4}, // 20: assignment (T_ID) ->  T_ID T_ASSIGN expression T_SEMICOLON
     {294, 5}, // 21: read (T_READ) ->  T_READ T_OPEN T_ID T_CLOSE T_SEMICOLON
     {295, 5}, // 22: write (T_WRITE) ->  T_WRITE T_OPEN expression T_CLOSE T_SEMICOLON
     {296, 5}, // 23: branch (T_IF) ->  T_IF expression T_BEGIN statements T_END
     {296, 9}, // 24: branch (T_IF) ->  T_IF expression T_BEGIN statements T_END T_ELSE T_BEGIN statements T_END
     {297, 5}, // 25: loop (T_WHILE) ->  T_WHILE expression T_BEGIN statements T_END
     {301, 1}, // 26: expression (T_NUM) ->  T_NUM
     {301, 1}, // 27: expression (T_TRUE) ->  T_TRUE
     {301, 1}, // 28: expression (T_FALSE) ->  T_FALSE
     {301, 1}, // 29: expression (T_ID) ->  T_ID
     {301, 3}, // 30: expression (T_ADD) ->  expression T_ADD expression
     {301, 3}, // 31: expression (T_SUB) ->  expression T_SUB expression
     {301, 3}, // 32: expression (T_MUL) ->  expression T_MUL expression
     {301, 3}, // 33: expression (T_DIV) ->  expression T_DIV expression
     {301, 3}, // 34: expression (T_MOD) ->  expression T_MOD expression
     {301, 3}, // 35: expression (T_LESS) ->  expression T_LESS expression
     {301, 3}, // 36: expression (T_GR) ->  expression T_GR expression
     {301, 3}, // 37: expression (T_EQ) ->  expression T_EQ expression
     {301, 3}, // 38: expression (T_AND) ->  expression T_AND expression
     {301, 3}, // 39: expression (T_OR) ->  expression T_OR expression
     {301, 2}, // 40: expression (T_NOT) ->  T_NOT expression
     {301, 3}, // 41: expression (T_OPEN) ->  T_OPEN expression T_CLOSE
     {302, 1}, // 42: program_$ ->  program
};
