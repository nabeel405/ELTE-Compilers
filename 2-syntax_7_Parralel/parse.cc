// Generated by Bisonc++ V5.02.00 on Tue, 03 Nov 2020 21:01:57 +0100
...
PI__ const s_productionInfo[] = 
{
     ...
     {298, 2}, // 10: statement (T_SEMICOLON) ->  read T_SEMICOLON
     {298, 2}, // 11: statement (T_SEMICOLON) ->  write T_SEMICOLON
     {298, 1}, // 12: statement ->  branch
     {298, 1}, // 13: statement ->  loop
     {298, 1}, // 14: statement ->  parallel
     {298, 1}, // 15: statement ->  read
     {298, 1}, // 16: statement ->  write
     {298, 1}, // 17: statement ->  assignment
     {304, 6}, // 18: parallel (T_SOPEN) ->  T_SOPEN statement T_PARALL statement T_SCLOSE T_SEMICOLON
     {299, 3}, // 19: assignment (T_ID) ->  T_ID T_ASSIGN expression
     {300, 4}, // 20: read (T_READ) ->  T_READ T_OPEN T_ID T_CLOSE
     {301, 4}, // 21: write (T_WRITE) ->  T_WRITE T_OPEN expression T_CLOSE
     {302, 5}, // 22: branch (T_IF) ->  T_IF expression T_THEN statements T_ENDIF
     {302, 7}, // 23: branch (T_IF) ->  T_IF expression T_THEN statements T_ELSE statements T_ENDIF
     {303, 5}, // 24: loop (T_WHILE) ->  T_WHILE expression T_DO statements T_DONE
     {305, 1}, // 25: expression (T_NUM) ->  T_NUM
     {305, 1}, // 26: expression (T_TRUE) ->  T_TRUE
     {305, 1}, // 27: expression (T_FALSE) ->  T_FALSE
     {305, 1}, // 28: expression (T_ID) ->  T_ID
     {305, 3}, // 29: expression (T_ADD) ->  expression T_ADD expression
     {305, 3}, // 30: expression (T_SUB) ->  expression T_SUB expression
     {305, 3}, // 31: expression (T_MUL) ->  expression T_MUL expression
     {305, 3}, // 32: expression (T_DIV) ->  expression T_DIV expression
     {305, 3}, // 33: expression (T_MOD) ->  expression T_MOD expression
     {305, 3}, // 34: expression (T_LESS) ->  expression T_LESS expression
     {305, 3}, // 35: expression (T_GR) ->  expression T_GR expression
     {305, 3}, // 36: expression (T_EQ) ->  expression T_EQ expression
     {305, 3}, // 37: expression (T_AND) ->  expression T_AND expression
     {305, 3}, // 38: expression (T_OR) ->  expression T_OR expression
     {305, 2}, // 39: expression (T_NOT) ->  T_NOT expression
     {305, 3}, // 40: expression (T_OPEN) ->  T_OPEN expression T_CLOSE
     {306, 1}, // 41: program_$ ->  program
};
